...
*   **Results Tracking Mandate**: Every quantitative result (speedup, area, power, frequency, IPC, throughput) from ingested papers must be extracted and cross-referenced in a master `results_tracker.md` in the wiki root. The tracker must annotate each result with its evidence strength (Strong/Moderate/Preliminary) and link back to the source wiki page. This serves as the single source of truth for which ideas work, at what cost, and with what confidence — enabling the team to make evidence-based architectural decisions.
*   **Mermaid Diagrams**: Use mermaid code blocks to clarify structure, flow, or results. Supported types: `graph`/`flowchart` for architecture and pipelines, `xychart-beta` for bar charts, `pie` for proportions, `gantt` for timelines. Keep diagrams simple — if it takes more than ~15 nodes, it's too complex. Base them strictly on source evidence. Never nest subgraphs deeply. Prefer simple pipelines over sprawling block diagrams.
