...
## Workflow
1. **Manifest & Read**: Read `sources/_index.md` to identify pending items (`- [ ]`). Parse raw source files and target directory `_index.md` files first.
2. **Synthesize**: Update or create a relevant note in `wiki/`.
   ...
   - **No Stubs (Strict)**: NEVER use, cite, or extract from sources with `status: stub` or failed extraction. Stop and ask the user for the full text PDF.
   ...
   - **Links**: Interconnect wiki pages using relative markdown links (no `[[wikilinks]]`).
3. **Audit Bloat**: Check if the target directory has > 15 content files (excluding `_index.md`). If so, reorganize per `AGENTS.md`.
4. **Indices & Clean**: Update target `_index.md` files (ensure links have one-line summaries and do not contain checkmarks). Remove the ingested item from `sources/_index.md`.
